interface ArgusLogoProps {
  ...
  className?: string;
  glow?: boolean;
  /** Cleaner emblem for favicons / home-screen tiles (matches navbar at small size). */
  icon?: boolean;
}

/** Panopticon emblem — one dominant all-seeing eye, four perimeter watchers. */
export function ArgusLogo({ size = 42, className = "", glow = true, icon = false }: ArgusLogoProps) {
  const uid = "argus";
  const showGlow = glow && !icon;
  return (
    <svg
      className={`argus-logo${showGlow ? " argus-logo--glow" : ""}${className ? ` ${className}` : ""}`}
      width={size}
      height={size}
      viewBox="0 0 64 64"
      fill="none"
    ...
    >
      ...
      {/* Outer watch ring */}
      <circle cx="32" cy="32" r="29" stroke={`url(#${uid}-frame)`} strokeWidth="1.4" opacity={icon ? 1 : 0.95} />
      ...
      {[
        ...
        <g key={i}>
          <circle cx={cx} cy={cy} r="2.2" fill="#020408" stroke="#00e5ff" strokeWidth="0.65" opacity={icon ? 1 : 0.75} />
          <circle cx={cx} cy={cy} r="0.75" fill="#ff1a4b" opacity={icon ? 1 : 0.95} />
        </g>
      ))}

      {/* Central eye */}
      <g filter={showGlow ? `url(#${uid}-glow)` : undefined}>
